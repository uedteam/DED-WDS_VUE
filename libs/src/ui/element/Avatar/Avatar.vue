...
<template>
  <div class="ded-avatar-wrapper">
    <div
      class="ded-avatar-container" :class="{
        [`ded-avatar-container-${props.size}`]: true,
        [ props.className ]: !!props.className,
      }"
    >
      <div class="ded-avatar" :class="[`ded-avatar-${props.shape}`]">
        <template v-if="props.src">
          <Image :src="props.src" :alt="props.alt" ratio="1x1" object-fit="cover" />
        </template>
        <template v-else>
          <span
            class="ded-avatar-text" :class="[`ded-text-${props.size}`]"
          >{{ getInitialsOrDefault(props.userName, 2)
          }}</span>
        </template>
      </div>
      <AvatarStatus :avatar-status="props.status" :avatar-size="props.size" />
    </div>
    <template v-if="props.isShowInfo">
      <div class="ded-avatar-info">
        <div class="ded-avatar-info-name">
          {{ props.userName }}
        </div>
        <div class="ded-avatar-info-caption">
          {{ props.caption }}
        </div>
      </div>
    </template>
