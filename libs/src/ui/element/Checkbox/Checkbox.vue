...
<template>
  <div
    class="ded-checkbox-container" :class="{
      [`ded-checkbox-container-${direction}`]: direction,
      [className]: !!className,
    }"
  >
    <label
      v-for="(item, index) in dataSource" :key="item.value" :for="generateId(index)"
      class="ded-checkbox"
      :class="{
    ...
    >
      <input
        :id="generateId(index)" class="ded-checkbox-input" type="checkbox" :name="item.name"
        :value="item.value" :checked="modelValue.includes(item.value)" v-bind="$attrs"
        @change="handleCheck(item)"
      >
      <!-- checkbox 選擇框樣式 -->
      <div
        class="ded-checkbox-icon" :class="[
          `ded-icon-${props.size}`,
