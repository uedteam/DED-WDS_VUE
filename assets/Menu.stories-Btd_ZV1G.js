import{_ as i}from"./Menu-BcaJH2a_.js";import"./vue.esm-bundler-CbCZpGld.js";import"./Icon-DutXaB3l.js";function d(a){return`[
        ${a.map(r=>`{
            label: '${r.label}',
            path: '${r.path}',
            prefix: '${r.prefix}',
      ...
      ></Menu>`}),parameters:{controls:{exclude:["navItemClick","expandedNav"]},docs:{source:{transform:(a,r)=>{const{args:e}=r,n=d(e.dataSource);return["<script setup>",'import { Menu } from "@ded-wds-vue/ui";',"<\/script>","","<template>","  <Menu",`    ${n?`:dataSource="${n}"`:""}`,`    ${e.isCollapsed!==void 0?`:isCollapsed="${e.isCollapsed}"`:""}`,`    ${e.color?`color="${e.color}"`:""}`,`    ${e.hasDivider!==void 0?`:hasDivider="${e.hasDivider}"`:""}`,`    ${e.className?`className="${e.className}"`:""}`,"  ></Menu>","</template>"].filter(Boolean).join(`
