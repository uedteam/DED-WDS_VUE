import{m as S,u as C,g as P,c as n,n as s,t as c,e as u,a as g,l as B,B as E,o as i}from"./vue.esm-bundler-Suj5BAdg.js";const _=["maxlength","placeholder"],t={__name:"Textarea",props:S({label:{type:String},placeholder:{type:String,default:"Placeholder"},limit:{type:Number,default:30},hint:{type:Object,default:()=>({error:"",description:""})},isDisabled:{type:Boolean,default:!1},className:{type:String,default:""}},{modelValue:{},modelModifiers:{}}),emits:["update:modelValue"],setup(a){const l=C(a,"modelValue"),e=a,r=P(()=>e.hint.error.length>0?"error":e.hint.description.length>0?"desc":"");return(j,p)=>(i(),n("div",{class:s({"ded-textarea-container":!0,[e.className]:!!e.className})},[e.label?(i(),n("label",{key:0,class:s(["ded-textarea-label",{"ded-textarea-disable":e.isDisabled}]),for:"id"},c(e.label),3)):u("",!0),g("div",{class:s(["ded-textarea-group",{"ded-textarea-disable":e.isDisabled},e.hint.error.length>0&&`ded-textarea-border-${r.value}`])},[B(g("textarea",{id:"id",class:s(["ded-textarea",{"ded-textarea-disable":e.isDisabled}]),maxlength:e.limit>0?e.limit:void 0,placeholder:e.placeholder,"onUpdate:modelValue":p[0]||(p[0]=y=>l.value=y)},null,10,_),[[E,l.value]]),l.value.length>0&&e.limit!==0?(i(),n("small",{key:0,class:s(["ded-textarea-hint-count",{"ded-textarea-disable":e.isDisabled}])},c(l.value.length>0?`${l.value.length} / ${e.limit}`:""),3)):u("",!0)],2),e.hint.error.length>0||e.hint.description.length>0?(i(),n("small",{key:1,class:s(["ded-textarea-hint",{"ded-textarea-disable":e.isDisabled},`ded-textarea-hint-${r.value}`])},c(e.hint.error.length>0?e.hint.error:e.hint.description),3)):u("",!0)],2))}};t.__docgenInfo={exportName:"default",displayName:"Textarea",description:"",tags:{},props:[{name:"label",type:{name:"string"}},{name:"placeholder",type:{name:"string"},defaultValue:{func:!1,value:'"Placeholder"'}},{name:"limit",type:{name:"number"},defaultValue:{func:!1,value:"30"}},{name:"hint",type:{name:"object"},defaultValue:{func:!1,value:`{
    ...
    description: ""
        ...
        description: '${a.description||""}'
            ...
            ></Textarea>
        `}),parameters:{controls:{},docs:{source:{transform:(a,l)=>{const{args:e}=l,r=V(e.hint);return["<script setup>",'import { ref } from "vue";','import Textarea from "@/ui/element/Textarea/Textarea.vue";',`const modelValue = ref("${e.modelValue}");`,"<\/script>","","<template>","  <Textarea",`    ${e.label?`label="${e.label}"`:""}`,`    ${e.placeholder?`placeholder="${e.placeholder}"`:""}`,`    ${e.limit!==void 0?`:limit="${e.limit}"`:""}`,`    ${r?`:hint="${r}"`:""}`,`    ${e.isDisabled!==void 0?`:isDisabled="${e.isDisabled}"`:""}`,`    ${e.className?`className="${e.className}"`:""}`,'    v-model="modelValue"',"  ></Textarea>","</template>"].filter(Boolean).join(`
    ...
    docs: {
      source: {
        transform: (src, storyContext) => {
          ...
          return [\`<script setup>\`, 'import { ref } from "vue";', \`import Textarea from "@/ui/element/Textarea/Textarea.vue";\`, \`const modelValue = ref("\${args.modelValue}");\`, \`<\/script>\`, '', '<template>', '  <Textarea', \`    \${args.label ? \`label="\${args.label}"\` : ""}\`, \`    \${args.placeholder ? \`placeholder="\${args.placeholder}"\` : ""}\`, \`    \${args.limit !== undefined ? \`:limit="\${args.limit}"\` : ""}\`, \`    \${dataSourceString ? \`:hint="\${dataSourceString}"\` : ""}\`, \`    \${args.isDisabled !== undefined ? \`:isDisabled="\${args.isDisabled}"\` : ""}\`, \`    \${args.className ? \`className="\${args.className}"\` : ""}\`, \`    v-model="modelValue"\`, '  ></Textarea>', '</template>'].filter(Boolean).join('\\n').trim();
        }
      }
    }
  }
}`,...(x=(h=o.parameters)==null?void 0:h.docs)==null?void 0:x.source}}};var $,f,T;d.parameters={...d.parameters,docs:{...($=d.parameters)==null?void 0:$.docs,source:{originalSource:`{
  name: "字數限制",
  args: {
    label: "Label",
    placeholder: "Placeholder",
...
}`,...(T=(f=d.parameters)==null?void 0:f.docs)==null?void 0:T.source}}};var D,N,v;m.parameters={...m.parameters,docs:{...(D=m.parameters)==null?void 0:D.docs,source:{originalSource:`{
  name: "輸入框狀態",
  args: {
    label: "Label",
    placeholder: "Placeholder",
    ...
    docs: {
      source: {
        transform: (src, storyContext) => {
          ...
          return [\`<script setup>\`, 'import { ref } from "vue";', \`import Textarea from "@/ui/element/Textarea/Textarea.vue";\`, \`const modelValue = ref("\${args.modelValue}");\`, \`<\/script>\`, '', '<template>', '  <Textarea', \`    \${args.label ? \`label="\${args.label}"\` : ""}\`, \`    \${args.placeholder ? \`placeholder="\${args.placeholder}"\` : ""}\`, \`    \${args.limit !== undefined ? \`:limit="\${args.limit}"\` : ""}\`, \`    :hint="{ error: '', description: 'Prompt message' }"\`, \`    \${args.isDisabled !== undefined ? \`:isDisabled="\${args.isDisabled}"\` : ""}\`, \`    \${args.className ? \`className="\${args.className}"\` : ""}\`, \`    v-model="modelValue"\`, '  ></Textarea>', '  <Textarea', \`    \${args.label ? \`label="\${args.label}"\` : ""}\`, \`    \${args.placeholder ? \`placeholder="\${args.placeholder}"\` : ""}\`, \`    \${args.limit !== undefined ? \`:limit="\${args.limit}"\` : ""}\`, \`    :hint="{ error: 'Error', description: '' }"\`, \`    \${args.isDisabled !== undefined ? \`:isDisabled="\${args.isDisabled}"\` : ""}\`, \`    \${args.className ? \`className="\${args.className}"\` : ""}\`, \`    v-model="modelValue"\`, '  ></Textarea>', '  <Textarea', \`    \${args.label ? \`label="\${args.label}"\` : ""}\`, \`    \${args.placeholder ? \`placeholder="\${args.placeholder}"\` : ""}\`, \`    \${args.limit !== undefined ? \`:limit="\${args.limit}"\` : ""}\`, \`    :hint="{ error: '', description: 'Prompt message' }"\`, \`    :isDisabled="true"\`, \`    \${args.className ? \`className="\${args.className}"\` : ""}\`, \`    v-model="modelValue"\`, '  ></Textarea>', '</template>'].filter(Boolean).join('\\n').trim();
        }
      }
    }
  }
}`,...(v=(N=m.parameters)==null?void 0:N.docs)==null?void 0:v.source}}};const A=["TextareaDefault","TextareaLimit","TextareaStatus"];export{o as TextareaDefault,d as TextareaLimit,m as TextareaStatus,A as __namedExportsOrder,k as default};
