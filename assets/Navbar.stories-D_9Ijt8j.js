import{_ as c}from"./Navbar-BN_UX73M.js";import"./vue.esm-bundler-CbCZpGld.js";import"./Avatar-DMoD7V1D.js";import"./Icon-DPZA6u3a.js";import"./Image-9BmbAKbZ.js";import"./Badge-o0YDJMvC.js";import"./Button-P8nFJln_.js";import"./Input-CGEMbczU.js";function l(r){return`[
    ${r.map(a=>`{
        label: '${a.label}',
        href: '${a.href}',
        order: '${a.order}',
    ...
    `)}
      ...
      ></Navbar>`}),parameters:{controls:{},docs:{source:{transform:(r,a)=>{const{args:o}=a;return["<script setup>",'import { Navbar } from "@ded-wds-vue/ui";',"<\/script>","","<template>","  <Navbar",`    :dataSource="${l(o.dataSource)||""}"`,`    ${o.hasLogo!==void 0?`:hasLogo="${o.hasLogo}"`:""}`,`    ${o.logoSrc?`logoSrc="${o.logoSrc}"`:""}`,`    ${o.logoLink?`logoLink="${o.logoLink}"`:""}`,`    ${o.avatarSrc?`avatarSrc="${o.avatarSrc}"`:""}`,`    ${o.className?`className="${o.className}"`:""}`,"  >","  </Navbar>","</template>"].filter(Boolean).join(`
    ...
    docs: {
      source: {
        transform: (src, storyContext) => {
          ...
          return [\`<script setup>\`, \`import { Navbar } from "@ded-wds-vue/ui";\`, \`<\/script>\`, "", "<template>", "  <Navbar", \`    :dataSource="\${dataSourceString || ""}"\`, \`    \${args.hasLogo !== undefined ? \`:hasLogo="\${args.hasLogo}"\` : ""}\`, \`    \${args.logoSrc ? \`logoSrc="\${args.logoSrc}"\` : ""}\`, \`    \${args.logoLink ? \`logoLink="\${args.logoLink}"\` : ""}\`, \`    \${args.avatarSrc ? \`avatarSrc="\${args.avatarSrc}"\` : ""}\`, \`    \${args.className ? \`className="\${args.className}"\` : ""}\`, "  >", "  </Navbar>", "</template>"].filter(Boolean).join("\\n").trim();
        }
      }
    }
  }
