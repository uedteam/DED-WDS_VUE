...
`).trim()}}}}},o={name:"元件尺寸",args:{size:"",placeholder:"Type something",isDisabled:!1,className:"",modelValue:""},render:a=>({components:{Search:i},setup(){return{args:a}},template:`
        ...
        `}),parameters:{controls:{exclude:["size"]},docs:{source:{transform:(a,s)=>{const{args:e}=s;return["<script setup>",'import { ref } from "vue";','import { Search } from "@ded-wds-vue/ui";','const modelValue = ref("");',"<\/script>","","<template>","  <Search",'    size="large"',`    ${e.placeholder?`placeholder="${e.placeholder}"`:""}`,`    ${e.isDisabled!==void 0?`:isDisabled="${e.isDisabled}"`:""}`,`    ${e.className?`className="${e.className}"`:""}`,'    v-model="modelValue"',"  ></Search>","  <Search",'    size="medium"',`    ${e.placeholder?`placeholder="${e.placeholder}"`:""}`,`    ${e.isDisabled!==void 0?`:isDisabled="${e.isDisabled}"`:""}`,`    ${e.className?`className="${e.className}"`:""}`,'    v-model="modelValue"',"  ></Search>","  <Search",'    size="small"',`    ${e.placeholder?`placeholder="${e.placeholder}"`:""}`,`    ${e.isDisabled!==void 0?`:isDisabled="${e.isDisabled}"`:""}`,`    ${e.className?`className="${e.className}"`:""}`,'    v-model="modelValue"',"  ></Search>","</template>"].filter(Boolean).join(`
`).trim()}}}}};var d,c,t;r.parameters={...r.parameters,docs:{...(d=r.parameters)==null?void 0:d.docs,source:{originalSource:`{
  name: "預設項目",
  args: {
    // themeColor: 'primary',
    size: "medium",
