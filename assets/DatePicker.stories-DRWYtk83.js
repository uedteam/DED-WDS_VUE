...
import { p as mergeModels, u as useModel, v as ref, w as watch, A as onMounted, D as nextTick, B as onUnmounted, c as createElementBlock, j as createBlock, b as createVNode, n as normalizeClass, o as openBlock } from "./vue.esm-bundler-K7CzQrxl.js";
...
function processOptions(options, datepicker) {
  ...
  Object.entries(inOpts).forEach(([key, value]) => {
    if (value !== void 0 && key in defaultOptions) {
      config[key] = value;
    }
  });
  return config;
  ...
  setup(__props) {
    ...
    function parseDate2(dateStr) {
      ...
      for (let i = 0; i < formatParts.length; i++) {
        ...
        if (formatParts[i].includes("m"))
          month = Number.parseInt(dateParts[i]) - 1;
        if (formatParts[i].includes("d"))
          day = Number.parseInt(dateParts[i]);
      }
      const newDate = new Date(year, month, day);
      return !Number.isNaN(newDate.getTime()) ? newDate : null;
    }
    ...
    function initSinglePicker() {
      ...
      if (dateSingleIns.value)
        return;
      if (!((_a2 = singleDateInputRef.value) == null ? void 0 : _a2.input))
        return;
      ...
      picker.element.addEventListener("changeDate", () => {
        ...
        modelValue.value = selectedDate ? formatDate2(selectedDate) : "";
      });
    ...
    function initRangePicker() {
      ...
      picker.element.addEventListener("changeDate", () => {
        ...
        if (Array.isArray(selectedDates) && selectedDates.length > 0) {
          const formattedDates = selectedDates.map(
            (date) => date instanceof Date ? formatDate2(date) : ""
          );
      ...
      if (Array.isArray(modelValue.value) && modelValue.value.length === 2) {
        const parsedDates = modelValue.value.map((dateStr) => parseDate2(dateStr));
    ...
    function clearRangeStart() {
      ...
      modelValue.value = ["", ((_a2 = modelValue.value) == null ? void 0 : _a2[1]) || ""];
    }
    function clearRangeEnd() {
      ...
      modelValue.value = [((_a2 = modelValue.value) == null ? void 0 : _a2[0]) || "", ""];
    }
      ...
      if (props.isRange) {
        if (dateRangeIns.value) {
          if (Array.isArray(newValue) && newValue.length === 2) {
            const parsedDates = newValue.map((dateStr) => parseDate2(dateStr));
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", {
        ...
        !props.isRange ? (openBlock(), createBlock(_sfc_main$1, {
          key: 0,
          ...
          prefix: "SvgCalendar",
          onClearDatePicker: _cache[1] || (_cache[1] = ($event) => clearSingleDate())
        }, null, 8, ["modelValue", "placeholder"])) : (openBlock(), createElementBlock("div", {
          key: 1,
          ...
          createVNode(_sfc_main$1, {
            ...
            "onUpdate:modelValue": _cache[2] || (_cache[2] = ($event) => modelValue.value[0] = $event),
            type: "text",
            placeholder: "Start Date",
            prefix: "SvgCalendar",
            onClearDatePicker: _cache[3] || (_cache[3] = ($event) => clearRangeStart())
          }, null, 8, ["modelValue"]),
          createVNode(_sfc_main$1, {
            ref: "endInputRef",
            modelValue: modelValue.value[1],
            "onUpdate:modelValue": _cache[4] || (_cache[4] = ($event) => modelValue.value[1] = $event),
            type: "text",
            placeholder: "End Date",
            prefix: "SvgCalendar",
            onClearDatePicker: _cache[5] || (_cache[5] = ($event) => clearRangeEnd())
          }, null, 8, ["modelValue"])
        ], 512))
      ], 2);
    };
...
_sfc_main.__docgenInfo = { "exportName": "default", "displayName": "DatePicker", "description": "", "tags": {}, "props": [{ "name": "format", "type": { "name": "string" }, "defaultValue": { "func": false, "value": '"yyyy/mm/dd"' } }, { "name": "isRange", "type": { "name": "boolean" } }, { "name": "placeholder", "type": { "name": "string" }, "defaultValue": { "func": false, "value": '""' } }, { "name": "className", "type": { "name": "string" }, "defaultValue": { "func": false, "value": '""' } }], "sourceFiles": ["/Users/auouser/Desktop/Work_Space/DED-WDS_VUE/libs/src/ui/element/DatePicker/DatePicker.vue"] };
    ...
    docs: {
      source: {
        transform: (src, storyContext) => {
          ...
          return [`<script setup>`, `import { ref } from "vue";`, `import { DatePicker } from "@ded-wds-vue/ui";`, `const modelValue = ref(${args.isRange ? "['','']" : '""'});`, `<\/script>`, "", "<template>", `<DatePicker`, `  ${args.format ? `format="${args.format}"` : ""}`, `  ${args.isRange !== void 0 ? `:isRange="${args.isRange}"` : ""}`, `  ${args.placeholder ? `placeholder="${args.placeholder}"` : ""}`, `  ${args.className ? `className="${args.className}"` : ""}`, `  v-model="modelValue"`, `></DatePicker>`, "</template>"].filter(Boolean).join("\n").trim();
        }
      }
    }
  }
...
DatePickerDefaultStory.parameters = {
  ...DatePickerDefaultStory.parameters,
  docs: {
    ...(_a = DatePickerDefaultStory.parameters) == null ? void 0 : _a.docs,
    source: {
      originalSource: '{\n  name: "預設項目",\n  args: {\n    format: "yyyy/mm/dd",\n    isRange: false,\n    placeholder: "YYYY/MM/DD",\n    className: "",\n    modelValue: ""\n  },\n  argTypes: {\n    modelValue: {\n      description: "日期",\n      control: {\n        type: "text"\n      },\n      table: {\n        type: {\n          summary: "string | string[]"\n        },\n        category: "v-model"\n      }\n    }\n  },\n  render: args => ({\n    components: {\n      DatePicker,\n      Input\n    },\n    setup() {\n      return {\n        args\n      };\n    },\n    template: `\n      <DatePicker\n        :format="args.format"\n        :isRange="args.isRange"\n        :placeholder="args.placeholder"\n        :className="args.className"\n        v-model="args.modelValue"\n      ></DatePicker>\n    `\n  }),\n  // 控制 controls 中能控制的參數\n  parameters: {\n    controls: {\n      // exclude: []\n    },\n    docs: {\n      source: {\n        transform: (src, storyContext) => {\n          const {\n            args\n          } = storyContext;\n          return [`<script setup>`, `import { ref } from "vue";`, `import { DatePicker } from "@ded-wds-vue/ui";`, `const modelValue = ref(${args.isRange ? "[\'\',\'\']" : "\\"\\""});`, `<\/script>`, "", "<template>", `<DatePicker`, `  ${args.format ? `format="${args.format}"` : ""}`, `  ${args.isRange !== undefined ? `:isRange="${args.isRange}"` : ""}`, `  ${args.placeholder ? `placeholder="${args.placeholder}"` : ""}`, `  ${args.className ? `className="${args.className}"` : ""}`, `  v-model="modelValue"`, `></DatePicker>`, "</template>"].filter(Boolean).join("\\n").trim();\n        }\n      }\n    }\n  }\n}',
      ...(_c = (_b = DatePickerDefaultStory.parameters) == null ? void 0 : _b.docs) == null ? void 0 : _c.source
    }
  }
};
DatePickerRange.parameters = {
  ...DatePickerRange.parameters,
  docs: {
    ...(_d = DatePickerRange.parameters) == null ? void 0 : _d.docs,
    source: {
      originalSource: '{\n  name: "日期區間",\n  args: {\n    format: "yyyy/mm/dd",\n    isRange: true,\n    className: "",\n    modelValue: ["", ""]\n  },\n  argTypes: {\n    modelValue: {\n      description: "日期",\n      control: {\n        type: "array"\n      },\n      table: {\n        type: {\n          summary: "string[]"\n        },\n        category: "v-model",\n        defaultValue: {\n          summary: "[\\"\\", \\"\\"]"\n        }\n      }\n    }\n  },\n  render: args => ({\n    components: {\n      DatePicker,\n      Input\n    },\n    setup() {\n      return {\n        args\n      };\n    },\n    template: `\n      <DatePicker\n        :format="args.format"\n        :isRange="args.isRange"\n        :className="args.className"\n        v-model="args.modelValue"\n      ></DatePicker>\n    `\n  }),\n  // 控制 controls 中能控制的參數\n  parameters: {\n    controls: {\n      // include: [ \'className\',\'modelValue\' ],\n      exclude: ["isRange", "placeholder"]\n    },\n    docs: {\n      source: {\n        transform: (src, storyContext) => {\n          const {\n            args\n          } = storyContext;\n          return [`<script setup>`, `import { ref } from "vue";`, `import { DatePicker } from "@ded-wds-vue/ui";`, `const modelValue = ref(${args.isRange ? "[\'\',\'\']" : "\\"\\""});`, `<\/script>`, "", "<template>", `<DatePicker`, `  ${args.format ? `format="${args.format}"` : ""}`, `  ${args.isRange !== undefined ? `:isRange="${args.isRange}"` : ""}`, `  ${args.placeholder ? `placeholder="${args.placeholder}"` : ""}`, `  ${args.className ? `className="${args.className}"` : ""}`, `  v-model="modelValue"`, `></DatePicker>`, "</template>"].filter(Boolean).join("\\n").trim();\n        }\n      }\n    }\n  }\n}',
      ...(_f = (_e = DatePickerRange.parameters) == null ? void 0 : _e.docs) == null ? void 0 : _f.source
    }
  }
};
