...
`).trim()}}}}},i={name:"輸入類型",args:{hasClear:!0,placeholder:"Placeholder",size:"medium",maxLimit:0,hint:{error:"",description:()=>s(n,{themeColor:"neutral",variant:"text",size:"medium",isShowDot:!1},"Prompt message")},isDisabled:!1,className:"",modelValueAccount:"Account",modelValuePassword:"Password",modelValueAmount:12345},render:a=>({components:{Input:o},setup(){return{args:a}},template:`
...
`).trim()}}}}},l={name:"提示訊息",args:{label:"Account",type:"text",hasClear:!0,placeholder:"Placeholder",size:"medium",prefix:"SvgAccount",maxLimit:0,hintError:{error:()=>s("div",{style:"display: flex; align-items: center; gap: 4px;"},[s(f,{name:"SvgErrorCircle",style:"color: red;",size:16}),s("span","Error message")]),description:""},hintPrompt:{error:"",description:()=>s(n,{themeColor:"neutral",variant:"text",size:"medium",isShowDot:!1},"Prompt message")},isDisabled:!1,className:"",modelValue:""},render:a=>({components:{Input:o},setup(){return{args:a,StatusIndicator:n}},template:`
      <div style="display: flex; flex-direction: column; gap:8px">
        <Input
          :label="args.label"
          :type="args.type"
    ...
    `}),parameters:{controls:{exclude:["hint","input","clearDatePicker"]},docs:{source:{transform:(a,r)=>{const{args:e}=r;return["<script setup>",'import { ref, h } from "vue";','import { Input, Icon, StatusIndicator } from "@ded-wds-vue/ui";','import Icon from "@ded-wds-vue/ui/Icon";','import StatusIndicator from "@ded-wds-vue/ui/StatusIndicator";','const modelValue = ref("");',"<\/script>","","<template>","  <Input",`    ${e.label?`label="${e.label}"`:""}`,`    ${e.type?`type="${e.type}"`:""}`,`    ${e.hasClear!==void 0?`:hasClear="${e.hasClear}"`:""}`,`    ${e.placeholder?`placeholder="${e.placeholder}"`:""}`,`    ${e.size?`size="${e.size}"`:""}`,`    ${e.prefix?`prefix="${e.prefix}"`:""}`,`    ${e.maxLimit?`maxLimit="${e.maxLimit}"`:""}`,`    :hint="{
    ...
    docs: {
      source: {
        transform: (src, storyContext) => {
          ...
          return [\`<script setup>\`, "import { ref } from \\"vue\\";", \`import { Input, StatusIndicator } from "@ded-wds-vue/ui";\`, "const modelValue = ref(\\"\\");", \`<\/script>\`, "", "<template>", "  <Input", \`    \${args.label ? \`label="\${args.label}"\` : ""}\`, \`    \${args.type ? \`type="\${args.type}"\` : ""}\`, \`    \${args.hasClear !== undefined ? \`:hasClear="\${args.hasClear}"\` : ""}\`, \`    \${args.placeholder ? \`placeholder="\${args.placeholder}"\` : ""}\`, \`    \${args.size ? \`size="\${args.size}"\` : ""}\`, \`    \${args.prefix ? \`prefix="\${args.prefix}"\` : ""}\`, \`    \${args.maxLimit ? \`maxLimit="\${args.maxLimit}"\` : ""}\`, \`    :hint="{
        error: '',
        description: () => h( StatusIndicator, {
          themeColor: 'neutral',
          variant: 'text',
    ...
    docs: {
      source: {
        transform: (src, storyContext) => {
          ...
          return [\`<script setup>\`, "import { ref, h } from \\"vue\\";", "import { Input, Icon, StatusIndicator } from \\"@ded-wds-vue/ui\\";", "import Icon from \\"@ded-wds-vue/ui/Icon\\";", "import StatusIndicator from \\"@ded-wds-vue/ui/StatusIndicator\\";", "const modelValue = ref(\\"\\");", \`<\/script>\`, "", "<template>", "  <Input", \`    \${args.label ? \`label="\${args.label}"\` : ""}\`, \`    \${args.type ? \`type="\${args.type}"\` : ""}\`, \`    \${args.hasClear !== undefined ? \`:hasClear="\${args.hasClear}"\` : ""}\`, \`    \${args.placeholder ? \`placeholder="\${args.placeholder}"\` : ""}\`, \`    \${args.size ? \`size="\${args.size}"\` : ""}\`, \`    \${args.prefix ? \`prefix="\${args.prefix}"\` : ""}\`, \`    \${args.maxLimit ? \`maxLimit="\${args.maxLimit}"\` : ""}\`, \`    :hint="{
        error: () => h('div', { style: 'display: flex; align-items: center; gap: 4px;' }, [
          h(Icon, { name: 'SvgErrorCircle', style: 'color: red;', size: 16 }),
          h('span', 'Error message')
        ]),
...
}`,...($=(x=l.parameters)==null?void 0:x.docs)==null?void 0:$.source}}};const z=["InputDefault","InputTypesStory","InputHintTypeStory"];export{t as InputDefault,l as InputHintTypeStory,i as InputTypesStory,z as __namedExportsOrder,C as default};
